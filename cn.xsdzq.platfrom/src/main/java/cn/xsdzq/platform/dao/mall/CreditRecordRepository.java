...
public interface CreditRecordRepository extends JpaRepository<CreditRecordEntity, Long> {
	...
	//跑批job用
	List<CreditRecordEntity> findByEndDateGreaterThanEqualAndEndDateLessThanEqualAndTypeAndChangeTypeIn(int startDate,int endDate, boolean type,List<Integer> list);
