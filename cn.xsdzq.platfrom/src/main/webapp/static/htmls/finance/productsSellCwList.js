...
function productsSellCwListController($scope, $http, $state, httpUtils, layerUtils, $stateParams, $gridService,utils) {
	...
	$scope.getWinPrizeList = function(pageSize) {
		var url = httpUtils.url.productsSellCwList;
		var clientId = "";
		var financeAccount = "";
		var productCode = "";
	...
	$scope.exportToExcel=function(){ 
		...
		var myDate = new Date().Format("yyyyMMddhhmmss");
		 alasql.promise('SELECT * INTO XLSX("场外产品销售数据表-' + myDate+ '.xlsx",{headers:true}) FROM ?',[excelArrs])
			.then(function (data) {
			  if(data == 1){
				$timeout(function(){
				  console.log('数据导出成功！');
