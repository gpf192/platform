...
function lcjPrizeListController($scope, $http, $state, httpUtils, layerUtils, utils) {
	...
	$scope.getPrizeList = function() {
		//1.请求存在的奖品列表
		var url = httpUtils.url.lcjPrizeList;
	...
	$scope.newBuild = function() {
		...
		if($scope.isModify) {
			...
			var url = httpUtils.url.modifyLcjPrize;
	...
	$scope.release = function() {
		...
		if($scope.newPrizeList.length >= 8) {
			...
			if(angular.toJson(customaryPrizeList) == angular.toJson($scope.newPrizeList)) {
				...
				//如果不一致提交更新
				var url = httpUtils.url.addLcjPrize;
	...
	$scope.deletePrize = function(index) {
		...
		layerUtils.iConfirm("是否需要删除奖品", function() {
			var url = httpUtils.url.deleteLcjPrize;
